...
#    print sess.graph.get_operations()
    folder = "3_GOT_GRUCell_seqlen" + str(SEQLEN) + "_batchsize" + str(BATCHSIZE) + "_intsize" + str(
    ...
    f = io.open("got_gru_samples.txt",'w+',encoding='utf-8');
...
#    print text_sample.decode('unicode-escape');
    ...
    while data.endValEpoch is False:
        ...
        feed_dict_val = {x: x_val_batch, y_true: y_val_batch, Hin: istate};
    ...
    while epoch <= max_epochs:
        ...
        feed_dict_train = {x: x_train_batch, y_true: y_train_batch, Hin: istate}
	...
	if step%100 == 0:
	    ...
	    saver.save(sess, 'models/'+folder+'/model');
            ...
            print("-----------------------------------------------------------");
            print("This is a text sample at step " + str(step));
	    f = io.open("got_gru_samples.txt",'a',encoding='utf-8');
            f.write(unicode("This is a text sample at step " + str(step) + ":\n"))
            text_sample = chr(random.randint(65, 90));
            xin = np.array([[ord(text_sample)]]);
            for k in range(SEQLEN*9):
                feed_dict = {x: xin, Hin: ist}
