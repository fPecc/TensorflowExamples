...
def encodeAlphabet(a):
    if a <= 255 and a >= 0:
        return a;
    elif a == 8212: # Es el guion largo que marca cuando habla un personaje.
        return 45;
    elif a == 8230:
        return 46; # Son los 3 puntos. Los devuelvo como un punto solo.
    else:
        return 0;
...
class Data:
    ...
    def loadText(self):
        ...
        print("Encoding data...");
        self.encodedTrainData = np.asarray([encodeAlphabet(ord(a)) for a in rawDataTrain]);
        self.encodedValData = np.asarray([encodeAlphabet(ord(a)) for a in rawDataVal]);
        return;
